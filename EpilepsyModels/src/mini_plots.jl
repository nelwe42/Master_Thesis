...
PK_timepoints = (wo_treatment+0.5):0.75:(Obs_Duration-0.5) #[0.2, 0.35, 1.05, 2.3, 3.7, 4.6, 5.75, 6.01, 7.5, 8.9]
...
pl = plot(xlabel="Time", ylabel="Concentration", tspan = (0, endpoint), ticks = false, left_margin=4Plots.mm)
plot!(sol, idxs = pk_model.keys.s[1], label="", tspan = (0,endpoint), linewidth = 3)
...
plot!(x_values, y_values, seriestype = :scatter, mc = :purple, markersize = 7, label = "", tspan = (0,endpoint))
display(pl)
