    ...
    try
        ...
        Threads.@threads for j in 1:min(individuals, thread_num)
            for i in ((j-1)*people_per_thread+1):min(j*people_per_thread, individuals)
                ...
                @inbounds loglikelihoods[i] = get_PK_loglikelihood(θ_use, data[i], sol=sol)
            end
        end
        if !keep_going[]
            return Inf
    ...
    if p.cont_seizure
        ...
        Threads.@threads for j in 1:min(individuals, thread_num)
            for i in ((j-1)*people_per_thread+1):min(j*people_per_thread, individuals)
                ...
                sol = solutions[i]
                @inbounds loglikelihoods[i] = get_seizure_loglikelihood(θ_use, m.seizure_model, sol, data[i], names=names)
                if !isfinite(loglikelihoods[i])
                    keep_going[] = false
                    break
                end
            end
...
function get_negloglikelihood_PK_vectorised(θ::AbstractVector, p::NamedTuple)
    θ_struct = ComponentArray(copy(θ), p.axes_θ)
    return get_negloglikelihood_PK(θ_struct, p)
end

function get_negloglikelihood_Seizure_vectorised(θ::AbstractVector, p::NamedTuple)
    θ_struct = ComponentArray(copy(θ), p.axes_θ)
    return get_negloglikelihood_Seizure(θ_struct, p)
end

function get_negloglikelihood_evaluated(θ::ComponentArray, m::FullModel, data::Tuple{Vararg{Person}}; logscale::Tuple{Vararg{String}} = (), max_threads::Int = 1, ODE_options::Tuple = (AutoTsit5(Rosenbrock23()),))
    ...
    p = (m = m, data = data, logscale = logscale, options = ODE_options, names=names, problems = problems, system = sys, indices_θ = indices_θ, max_threads = max_threads, cont_seizure = (m.seizure_model isa SeizureModelContinuous))
    negloglikeli = get_negloglikelihood(θ_use, p)
    return negloglikeli
end

function get_negloglikelihood_evaluated_hierarchical(θ::ComponentArray, m::FullModel, data::Tuple{Vararg{Person}}; logscale::Tuple{Vararg{String}} = (), max_threads::Int = 1, ODE_options::Tuple = (AutoTsit5(Rosenbrock23()),))
    max_threads = max(max_threads, 1)
    names = get_keys_PK(m.pk_model)
    sys = create_ode_system(m.pk_model)
    problems = Tuple(create_problem(m.pk_model, sys, person=person, endpoint = max(person.measurements[end].timepoint, (person.seizure_counts[end].time isa Tuple ? person.seizure_counts[end].time[2] : person.seizure_counts[end].time))) for person in data)
    ...
    partial_transform_to_logscale!(θ_use, logscale = logscale)
    p_PK = (data = data, logscale = logscale, options = ODE_options, problems = problems, system = sys, indices_θ = indices_θ, max_threads = max_threads, axes_θ = getaxes(θ_use))
    p_Seizure = (m = m, data = data, logscale = logscale, names = names, solutions = solutions, max_threads = max_threads, cont_seizure = (m.seizure_model isa SeizureModelContinuous))
...
function optimise_hierarchical(m::FullModel, data::Tuple{Vararg{Person}}; maxiters::Int64 = 10^4, maxtime::AbstractFloat = Inf, logscale::Tuple{Vararg{String}} = (), run_CI::Bool = false, bound_abs::Union{Nothing, AbstractFloat} = nothing, lower_upper::Union{Nothing, Tuple{ComponentArray, ComponentArray}} = nothing, 
    ...
    θ_0 = ComponentArray((PK = deepcopy(m.pk_model.θ), Seizure = deepcopy(m.seizure_model.θ))) 
    ...
    axes_θ = getaxes(θ_0)
    axes_PK = getaxes(θ_0_PK)
    axes_Seizure = getaxes(θ_0_Seizure)
    θ_0_vec = collect(θ_0)
    d = length(θ_0_vec)
    ...
    p_PK = (data = data, logscale = logscale, options = ODE_options, problems = problems, system = sys, indices_θ = indices_θ, axes_θ = axes_PK, objective_fail_hard = objective_fail_hard, max_threads = internal_threads, objective_warn = objective_warn, objective_warned_ref = Ref(false))
    if vectorised
        objective_PK = OptimizationFunction(get_negloglikelihood_PK_vectorised, Optimization.AutoForwardDiff())
        #Change everything to vectors
        θ_0 = θ_0_vec
        objective_Seizure = OptimizationFunction(get_negloglikelihood_Seizure_vectorised, Optimization.AutoForwardDiff())
        if !isnothing(ub)
            ub_PK = collect(ub.PK)
            lb_PK = collect(lb.PK)
            ub_Seizure = collect(ub.Seizure)
            lb_Seizure = collect(lb.Seizure)
            ub = collect(ub)
            lb = collect(lb)
        else
            ub_PK = nothing
        ...
        if !isnothing(ub)
            ub_PK = ub.PK
            lb_PK = lb.PK
            ub_Seizure = ub.Seizure
            lb_Seizure = lb.Seizure
        end
    end
    ...
    if vectorised
        ...
        starts_list = [ComponentArray(vec(starts[i, :]), axes_θ) for i in 1:n_starts]
    end
    ...
    #Split starts into PK and Seizure
    n_starts = max(1,multistart)
    if vectorised
        starts_list = [ComponentArray(start, axes_θ) for start in starts_list]
        starts_list_PK = [collect(start.PK) for start in starts_list]
        starts_list_Seizure = [collect(start.Seizure) for start in starts_list]
    else
        starts_list_PK = [ComponentArray(collect(start.PK), axes_PK) for start in starts_list]
        starts_list_Seizure = [ComponentArray(collect(start.Seizure), axes_Seizure) for start in starts_list]
    end
    ...
    solutions_PK = Array{SciMLBase.AbstractNoTimeSolution}(undef, n_starts)
    solutions_Seizure = Array{Union{SciMLBase.AbstractNoTimeSolution, Nothing}}(undef, n_starts)
    ...
    Threads.@threads for j in 1:min(n_starts, thread_num)
        for i in ((j-1)*starts_per_thread+1):min(j*starts_per_thread, n_starts)
            #First fit PK
            problem_PK = OptimizationProblem(objective_PK, starts_list_PK[i], p_PK, lb = lb_PK, ub = ub_PK)
            solutions_PK[i] = solve(problem_PK, solver_optim, maxiters = maxiters, maxtime = maxtime, store_trace = store_trace, solver_options...)
            #transform parameters back into non logscale
            if vectorised
                solutions_PK_u = ComponentArray(solutions_PK[i].u, axes_PK)
            else
                solutions_PK_u = deepcopy(solutions_PK[i].u)
            end
            partial_transform_to_logscale_partwise!(solutions_PK_u, logscale = logscale, detransform = true)
            #Check ODE solves successful
            individuals = length(data)
            thread_num = p_PK.max_threads
            solutions = Array{ODESolution}(undef, individuals)
            people_per_thread = Int(ceil(individuals/thread_num))
            Threads.@threads for l in 1:min(individuals, thread_num)
                for k in ((l-1)*people_per_thread+1):min(l*people_per_thread, individuals)
                    solutions[k] = solve_PK(problems[k], sys, solutions_PK_u, indices_θ = indices_θ, options = ODE_options) 
                end
            end
            if !SciMLBase.successful_retcode(solutions_PK[i].retcode) || any(.!(SciMLBase.successful_retcode.(solutions)))
                @warn "Unsuccessful solve in PK estimation, disregarding this start"
                solutions_Seizure[i] = nothing
            else
                #Fit Seizure model
                p_Seizure = (m = m, data = data, logscale = logscale, names = names, solutions = solutions, cont_seizure = (m.seizure_model isa SeizureModelContinuous), axes_θ = axes_Seizure, max_threads = internal_threads)
                problem_Seizure = OptimizationProblem(objective_Seizure, starts_list_Seizure[i], p_Seizure, lb = lb_Seizure, ub = ub_Seizure)
                solutions_Seizure[i] = solve(problem_Seizure, solver_optim, maxiters = maxiters, store_trace = store_trace, solver_options...) 
            end
        end
    end
    ...
    for i in 1:n_starts
        if isnothing(best_raw_any)
            best_raw_any = (solutions_PK[i], solutions_Seizure[i])
            best_start_any = i
        end
        if isnothing(solutions_Seizure[i])
            finite_i = false
        else
            finite_i = isfinite(solutions_PK[i].objective + solutions_Seizure[i].objective)
        end
        if finite_i
            obj_i = solutions_PK[i].objective + solutions_Seizure[i].objective
            if isnothing(best_raw_finite) || obj_i < best_obj_finite
                best_raw_finite = (solutions_PK[i], solutions_Seizure[i])
                best_obj_finite = obj_i
                best_start_finite = i
            end
            if SciMLBase.successful_retcode(solutions_PK[i].retcode) && SciMLBase.successful_retcode(solutions_Seizure[i].retcode) && (isnothing(best_raw_success_finite) || obj_i < best_obj_success_finite)
                best_raw_success_finite = (solutions_PK[i], solutions_Seizure[i])
    ...
    if vectorised
        if !isnothing(estimate_raw[2])
            estimate_u = ComponentArray(vcat(estimate_raw[1].u, estimate_raw[2].u), axes_θ)
            partial_transform_to_logscale!(estimate_u, logscale=logscale, detransform = true)
        else
            estimate_u = ComponentArray(PK = ComponentVector(estimate_raw[1].u, axes_PK), Seizure = nothing)
            partial_transform_to_logscale_partwise!(estimate_u.PK, logscale=logscale, detransform=true)
        end
    else 
        if !isnothing(estimate_raw[2])
            estimate_u = ComponentArray(PK = estimate_raw[1].u, Seizure = estimate_raw[2].u)
            partial_transform_to_logscale!(estimate_u, logscale=logscale, detransform = true)
        else
            estimate_u = ComponentArray(PK = estimate_raw[1].u, Seizure = nothing)
            partial_transform_to_logscale_partwise!(estimate_u.PK, logscale=logscale, detransform=true)
        end
    end
    estimate_PK = estimate_raw[1]
    estimate_Seizure = estimate_raw[2]

    for i in 1:n_starts
        if !isnothing(solutions_Seizure[i]) && !(SciMLBase.successful_retcode(solutions_PK[i].retcode) && SciMLBase.successful_retcode(solutions_Seizure[i].retcode)) && ((solutions_PK[i].objective + solutions_Seizure[i].objective) < estimate_raw.objective)
            ...
            println(solutions_PK[i].original)
            if !isnothing(solutions_Seizure[i])
                println(solutions_Seizure[i].original)
            end
        end
    end
    
    if run_CI && !isnothing(estimate_Seizure)
        ...
        estimate = (u = estimate_u, retcode = (isnothing(estimate_Seizure) ? false : estimate_Seizure.retcode), objective = (PK = estimate_PK.objective, Seizure = estimate_Seizure.objective), estimate_PK = estimate_PK, estimate_Seizure = estimate_Seizure, CI = CI)
    else
        if run_CI
            @warn "No CI computed due to missing seizure estimation"
        end
        estimate = (u = estimate_u, retcode = (isnothing(estimate_Seizure) ? false : estimate_Seizure.retcode), objective = (PK = estimate_PK.objective, Seizure = (isnothing(estimate_Seizure) ? Inf : estimate_Seizure.objective)), estimate_PK = estimate_PK, estimate_Seizure = estimate_Seizure)
    end
    if printing
